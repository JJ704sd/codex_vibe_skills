...
After each round, checkpoint the pinned inputs, confirmed decisions, accepted assumptions, and unresolved frontier. If two consecutive rounds add no evidence and neither close nor narrow a decision, stop before repeating a question and report the minimum unresolved decision or evidence.
...
A discussion never authorizes implementation or an external mutation. Write confirmed outcomes to a repository document only when the user requests durable documentation and identifies or approves the destination.
