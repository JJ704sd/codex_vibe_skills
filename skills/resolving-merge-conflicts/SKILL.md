...
1. Inspect `git status`, the merge or rebase state, recent history, conflicting paths, staged content, and unrelated working-tree changes. Pin the current `HEAD`, operation type, and unmerged-path set, then group conflicts that share a contract or dependency.
...
If `HEAD`, the active operation, or the unmerged-path set changes after analysis, re-inspect before editing or staging. Never apply a stale resolution plan to a changed conflict state.
