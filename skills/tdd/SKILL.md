...
Do not write the entire test suite first or implement layer by layer. Let each completed slice inform the next. Do not enter green without the intended red; stop when another attempt would repeat the same unrelated failure without new evidence.
...
If the task is behavior-preserving restructuring rather than new behavior, use `$refactoring-safely`; do not invent a red state.
