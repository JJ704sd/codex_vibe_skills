...
| 项目 | 官方机制 | 对本仓库的适配结论 |
| --- | --- | --- |
| [LangGraph workflows](https://langchain-ai.github.io/langgraph/agents/tools/) / [persistence](https://langchain-ai.github.io/langgraph/concepts/time-travel/) | 条件边、并行节点、orchestrator-worker、evaluator-optimizer、持久化与 human-in-the-loop | graph 只表达真实依赖；客观 evaluator 决定回路；长迁移在安全阶段做 checkpoint |
...
| [FastContext](https://github.com/microsoft/fastcontext) | 独立的只读仓库探索节点，返回精确文件和行号而非污染主求解上下文 | 大仓库可并行定位，但只返回可复核的 context capsule；小仓库不分派 |
